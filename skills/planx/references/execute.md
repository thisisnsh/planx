...
`planx list --approved --json` and pick from the titles; if it is ambiguous, ask.
...
approved — changing it is the user's decision, not yours.

## Do not edit the plan

Executing a plan does not revise it. If it needs changing, that is a planning
round: the plan is likely sealed and every section locked, so a change goes
through review.
